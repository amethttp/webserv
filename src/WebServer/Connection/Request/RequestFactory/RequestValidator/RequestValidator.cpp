...
#include "WebServer/Connection/Request/RequestFactory/RequestPctDecoder/RequestPctDecoder.hpp"
...
SimpleResult RequestValidator::validateRequestLine(const RequestLine &requestLine)
{
    if (requestLine.getMethod() == M_NOT_IMPLEMENTED)
        return SimpleResult::fail(NOT_IMPLEMENTED);

    if (requestLine.getTargetUri().length() > MAX_URI_LENGTH)
        return SimpleResult::fail(REQUEST_URI_TOO_LARGE);

    if (requestLine.getHttpVersion() != "HTTP/1.1")
        return SimpleResult::fail(HTTP_VERSION_NOT_SUPPORTED);

    if (!RequestPctDecoder::isWellEncoded(requestLine.getTargetUri()))
        return SimpleResult::fail(BAD_REQUEST);
