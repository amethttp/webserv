...
Result<t_Request> RequestFactory::create(const std::string &requestString)
{
    t_Request request;
    ...
    if (requestLineResult.isFailure())
        return Result<t_Request>::fail(requestLineResult.getError());
    ...
    if (requestHeadersResult.isFailure())
        return Result<t_Request>::fail(requestHeadersResult.getError());
    ...
    if (requestBodyResult.isFailure())
        return Result<t_Request>::fail(requestBodyResult.getError());
    request.body = requestBodyResult.getValue();

    return Result<t_Request>::ok(request);
}
