...
bool Server::matchesPort(const std::string &match)
{
	int port;
	std::stringstream sstream(match);

	sstream >> port;
	for (std::set<int>::iterator name = this->ports_.begin(); name != this->ports_.end(); ++name)
	{
		if (*name == port)
			return true;
	}
